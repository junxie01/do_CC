...
integer year_b,year_e,day_b,day_e,cn
...
if (iargc().ne.1)then
   ...
   write(*,*)'cn append dsec multpt com nlen dorot'
...
read(10,*)cn,append,dsec,multpt,comm,npt2,dorot
read(10,*)dirinn
read(10,*)dirout
close(10)
      ...
      do iseg=1,nseg                     ! loop over each segment
         ...
         do is1=1,nst1                   ! loop over station 1
            do ic1=1,3                   ! check whether all three components exist
               write(sacfile1(ic1),'(1a,"/",1a,"/",1a,"_",i2.2,"_",i2.2,"_",i2.2,"_",1a,"_",1a,"_",1a,".SAC_",1a)')&
               trim(dirinn),trim(year_day),trim(year_day),nzhour,nzmin,nzsec,&
               trim(net1(is1)),trim(sta1(is1)),trim(com(ic1)),trim(append)
               if(trim(append)=="null")&
            ...
            do is2=1,nst2      ! loop over station 2
               ...
               do ic2=1,cn      ! check whether all three components of station two exist
                  write(sacfile2(ic2),'(1a,"/",1a,"/",1a,"_",i2.2,"_",i2.2,"_",i2.2,"_",1a,"_",1a,"_",1a,".SAC_",1a)')&
                  trim(dirinn),trim(year_day),trim(year_day),nzhour,nzmin,nzsec,&
                  trim(net2(is2)),trim(sta2(is2)),trim(com(ic2)),trim(append)
                  if(trim(append)=="null")&
               ...
               if(dorot.eq.1.and.cn.eq.3)then
