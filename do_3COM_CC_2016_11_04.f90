...
character (180)output_ncc,output_pcc,output
...
do is=1,nst   ! loop over station pair
   ...
   trim(dirout),trim(net1(is)),trim(sta1(is)),trim(net1(is)),trim(sta1(is)),trim(net2(is)),trim(sta2(is)) ! mkdir for outdir
   numcc=0   ! num of cc
      ...
      do id=begday,endday ! loop over day
         ...
         do ih=1,nh      ! loop over hour segment
            ...
            do ic1=1,cn                     ! check whether all three components exist
               ...
               if (append.eq."null")sac1(ic1)=trim(dirinn)//'/'//trim(year_day)//'/'//trim(year_day)//"_"//trim(nd)//&
            ...
            do ic2=1,cn     ! check whether all three components of station two exist
               ...
               if(append.eq."null")sac2(ic2)=trim(dirinn)//'/'//trim(year_day)//'/'//trim(year_day)//"_"//trim(nd)//&
            ...
            if(sachead2%delta.ne.sachead1%delta)cycle
            dt=sachead1%delta
            nsamp=sachead1%npts
            ...
            do ic1=1,cn                     ! loop over com1
               do ic2=1,cn                  ! loop over com2
                  ...
                  do iseg=1,nseg
                     ...
                     write(output_ncc,'(1a,"/",1a,"_",1a,"/",1a,"_",1a,"_",1a,"_",1a,"/ncc_",1a,"_",1a,"_",1a,"_",1a,"_",&
                     1a,"_",1a,"_",1a,"_",1a,".SAC_",i2.2)')trim(dirout),trim(net1(is)),trim(sta1(is)),trim(net1(is)),&
                     trim(sta1(is)),trim(net2(is)),trim(sta2(is)),trim(year_day),trim(nd),trim(net1(is)),&
                     trim(sta1(is)),trim(net2(is)),trim(sta2(is)),trim(com(ic1)),trim(com(ic2)),iseg

                     write(output_pcc,'(1a,"/",1a,"_",1a,"/",1a,"_",1a,"_",1a,"_",1a,"/pcc_",1a,"_",1a,"_",1a,"_",1a,"_",&
                     1a,"_",1a,"_",1a,"_",1a,".SAC_",i2.2)')trim(dirout),trim(net1(is)),trim(sta1(is)),trim(net1(is)),&
                     trim(sta1(is)),trim(net2(is)),trim(sta2(is)),trim(year_day),trim(nd),trim(net1(is)),&
                     trim(sta1(is)),trim(net2(is)),trim(sta2(is)),trim(com(ic1)),trim(com(ic2)),iseg
                     ...
                     if(doncc.eq.1.and.dotl.ne.1)call write_ncf_sac(output_ncc,signcc,sachead1,sachead2,nsmpl,1,nerr)
                     if(dopcc.eq.1.and.dotl.ne.1)call write_ncf_sac(output_pcc,sigpcc,sachead1,sachead2,nsmpl,1,nerr)
   ...
   do ic1=1,cn
      do ic2=1,cn
         if(numcc(ic1,ic2).gt.0)then
            if(dotl.eq.1)then
               if(doncc.eq.1)then
                  write(output,'(1a,"/",1a,"_",1a,"/",1a,"_",1a,"_",1a,"_",1a,"/tl_ncc_COR_",1a,"_",1a,"_",1a,"_",1a,"_",&
                  1a,"_",1a,"_",1a,".SAC")')trim(dirout),trim(net1(is)),trim(sta1(is)),trim(net1(is)),trim(sta1(is)),&
            trim(net2(is)),trim(sta2(is)),trim(name),trim(net1(is)),trim(sta1(is)),trim(net2(is)),trim(sta2(is)),com(ic1),com(ic2)
                  call write_ncf_sac(output,outncctl(:,ic1,ic2),sachead1,sachead2,nsmpl,numcc(ic1,ic2),nerr)
               endif
               if(dopcc.eq.1)then
                  write(output,'(1a,"/",1a,"_",1a,"/",1a,"_",1a,"_",1a,"_",1a,"/tl_pcc_COR_",1a,"_",1a,"_",1a,"_",1a,"_",&
                  1a,"_",1a,"_",1a,".SAC")')trim(dirout),trim(net1(is)),trim(sta1(is)),trim(net1(is)),trim(sta1(is)),&
                  trim(net2(is)),trim(sta2(is)),trim(name),trim(net1(is)),trim(sta1(is)),trim(net2(is)),trim(sta2(is)),&
                  trim(com(ic1)),trim(com(ic2))
            ...
            if(dopw.eq.1)then
               if(doncc.eq.1)then
                  ...
                  enddo
                  write(output,'(1a,"/",1a,"_",1a,"/",1a,"_",1a,"_",1a,"_",1a,"/pw_ncc_COR_",1a,"_",1a,"_",1a,"_",1a,"_",&
                  1a,"_",1a,"_",1a,".SAC")')trim(dirout),trim(net1(is)),trim(sta1(is)),trim(net1(is)),trim(sta1(is)),&
                  trim(net2(is)),trim(sta2(is)),trim(name),trim(net1(is)),trim(sta1(is)),trim(net2(is)),trim(sta2(is)),&
                  trim(com(ic1)),trim(com(ic2))
               ...
               if(dopcc.eq.1)then
                  ...
                  enddo
                  write(output,'(1a,"/",1a,"_",1a,"/",1a,"_",1a,"_",1a,"_",1a,"/pw_pcc_COR_",1a,"_",1a,"_",1a,"_",1a,"_",&
                  1a,"_",1a,"_",1a,".SAC")')trim(dirout),trim(net1(is)),trim(sta1(is)),trim(net1(is)),trim(sta1(is)),&
                  trim(net2(is)),trim(sta2(is)),trim(name),trim(net1(is)),trim(sta1(is)),trim(net2(is)),trim(sta2(is)),&
                  trim(com(ic1)),trim(com(ic2))
