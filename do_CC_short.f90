...
write(*,'("Dsec1=",1i7)')dsec1
write(*,'("Dsec2=",1i7)')dsec2
      ...
      do id=begday,endday ! loop over day
         ...
         do iseg1=1,nseg1  ! loop over each segment
            ...
            do ic1=1,cn                     ! check whether all three components exist
               ...
               trim(year_day),trim(year_day),nzhour,nzmin,nzsec,trim(net1(is)),trim(sta1(is)),trim(com(ic1)),trim(append)
               if(append.eq."null")then
                  write(sacfile,'(1a,"/",1a,"/",1a,"_",i2.2,"_",i2.2,"_",i2.2,"_",1a,"_",1a,"_",1a,".SAC")')&
                  trim(dirinn),trim(year_day),trim(year_day),nzhour,nzmin,nzsec,trim(net1(is)),trim(sta1(is)),trim(com(ic1))
               endif
               sac1(ic1)=sacfile
 ...
 !100        continue
            if(it.eq.cn)then   ! if all three components of station 1 exist
               do ic2=1,cn     ! check whether all three components of station two exist
                  write(sacfile,'(1a,"/",1a,"/",1a,"_",i2.2,"_",i2.2,"_",i2.2,"_",1a,"_",1a,"_",1a,".SAC_",1a)')trim(dirinn),&
                  trim(year_day),trim(year_day),nzhour,nzmin,nzsec,trim(net2(is)),trim(sta2(is)),trim(com(ic2)),trim(append)
                  if(append.eq."null")then
                     write(sacfile,'(1a,"/",1a,"/",1a,"_",i2.2,"_",i2.2,"_",i2.2,"_",1a,"_",1a,"_",1a,".SAC")')&
                     trim(dirinn),trim(year_day),trim(year_day),nzhour,nzmin,nzsec,trim(net2(is)),trim(sta2(is)),trim(com(ic2))
                  endif
                  sac2(ic2)=sacfile
...
call cpu_time(t2)
write(*,'("Segment length:",i5.5,"sec Overlaping:",i2.2,"%, Time costs: ",f17.3,"s")')dsec2,multpt1,t2-t1
end program
